...
from sina.items import TweetsItem, InformationItem, RelationshipsItem, CommentItem, RepostItem
...
class WeiboSpider(Spider):
    ...
    def start_requests(self):
        ...
        # test luoyunxi
        # yield Request(url='https://weibo.cn/1860563805/info', callback=self.parse_information)
        ...
        # original weibo posted by 共青团: https://weibo.cn/repost/H3GQixVWe?uid=3937348351&page=1
        # luo repost: https://weibo.cn/repost/H3J6MeySN?uid=1860563805&rl=0
        weibo_url = 'https://weibo.cn/repost/{}?uid={}&page=1'.format(wbid, uid)
        yield Request(url=weibo_url, callback=self.parse_reposts, meta={'org_wbid_uid': '{}_{}'.format(wbid, uid)})
    ...
    def parse_reposts(self, response):
        if response.url.endswith('page=1'):
            ...
            if all_page:
                ...
                # for page_num in range(2, 3):
                    ...
                    yield Request(page_url, self.parse_reposts, dont_filter=True, meta=response.meta)
        """
        解析本页的数据
        """

        selector = Selector(response)
        repost_nodes = selector.xpath('//div[not(@id) and @class="c"]')
        # /html/body/div[8]
        ...
        for repost_node in repost_nodes:
            ...
            repost_item['org_wbid_uid'] = response.meta['org_wbid_uid']
            repost_item['repost_user_id'] = re.search(r'/u/(\d+)', repost_user_url).group(1)
            repost_item['content'] = repost_node.xpath('string(.)').extract_first()
            created_at = repost_node.xpath('.//span[@class="ct"]/text()').extract_first()

            # because we scraped an old weibo, datetime format = 2018-11-21 19:57:06
            # could just split 来... but it seems correct so nevermind lol 
            try:           
                if len(created_at.split('\xa0')) == 4:
                    repost_item['created_at'] = created_at.split('\xa0')[:2]
                    repost_item['repost_device'] = created_at.split('\xa0')[3]
                else:
                    repost_item['created_at'] = time_fix(created_at.split('\xa0')[0])
                    repost_item['repost_device'] = created_at.split('\xa0')[2]
            except Exception as e:
                self.logger.error(e)

            yield repost_item

            yield Request(url="https://weibo.cn/%s/info" % repost_item['repost_user_id'], callback=self.parse_information) 
    ...
    def parse_tweet(self, response):
        ...
        tweet_nodes = tree_node.xpath('//div[@class="c" and @id]') # each weibo post node
